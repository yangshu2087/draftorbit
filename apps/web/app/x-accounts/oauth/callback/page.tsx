...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    if (!state || !code) {
      ...
      const t = setTimeout(() => router.replace('/x-accounts'), 2200);
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setMessage('绑定成功，正在返回账号页…');
          setTimeout(() => router.replace('/x-accounts'), 400);
        }
      } catch (e) {
        if (cancelled) return;
        setPhase('error');
        setMessage(e instanceof Error ? e.message : '绑定失败');
        setTimeout(() => router.replace('/x-accounts'), 2500);
      }
    })();
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-white px-4">
      ...
      {phase === 'error' ? <p className="text-center text-xs text-slate-400">即将返回账号页…</p> : null}
    </div>
  );
}
