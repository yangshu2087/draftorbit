...
test('buildV4StudioRunRequest normalizes diagram into background visual request', () => {
...
test('V4 preview copy uses user-facing asset labels and never presents failed assets as real', () => {
  const preview = buildV4StudioPreview({
    textResult: { format: 'article', content: '标题\n\n导语', variants: [] },
    visualAssets: [
      { id: 'cover', kind: 'cover', status: 'ready', provider: 'codex-local-svg', exportFormat: 'svg', provenanceLabel: 'SVG 图文资产', signedAssetUrl: '/asset.svg' },
      { id: 'failed', kind: 'infographic', status: 'failed', provider: 'template-svg', exportFormat: 'svg', provenanceLabel: '导出资产' }
    ],
    sourceArtifacts: [],
  ...
  assert.equal(preview.readyAssets[0]?.providerLabel, 'SVG 图文资产');
...
test('V4 provider labels stay ordinary-user friendly', () => {
  assert.equal(getV4ProviderLabel('codex-local-svg'), 'SVG 图文资产');
  assert.equal(getV4ProviderLabel('ollama-text'), '本地草稿资产');
  assert.equal(getV4ProviderLabel('template-svg'), '导出资产');
});
...
test('V4 preview only enables bundle download when a real signed asset URL exists', () => {
  const localPreview = buildV4StudioPreview({
    ...
    visualAssets: [
      { id: 'local-card', kind: 'cards', status: 'ready', provider: 'codex-local-svg', exportFormat: 'svg', provenanceLabel: 'SVG 图文资产' }
    ],
    sourceArtifacts: [],
    qualityGate: { status: 'passed', safeToDisplay: true, hardFails: [] },
    publishPreparation: { mode: 'manual-confirm', label: '准备发布 / 手动确认', canAutoPost: false },
  ...
  assert.equal(localPreview.bundleActionCopy, '结果完成后可下载导出包');
  ...
  const runPreview = buildV4StudioPreview({
    ...
    visualAssets: [
      { id: 'card', kind: 'cards', status: 'ready', provider: 'codex-local-svg', exportFormat: 'svg', signedAssetUrl: '/v3/chat/runs/run/assets/card?token=signed', provenanceLabel: 'SVG 图文资产' }
    ],
    sourceArtifacts: [],
    qualityGate: { status: 'passed', safeToDisplay: true, hardFails: [] },
    publishPreparation: { mode: 'manual-confirm', label: '准备发布 / 手动确认', canAutoPost: false },
  ...
  assert.equal(runPreview.bundleActionCopy, '下载导出包');
});
